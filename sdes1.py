...
def in_to_bin(k_input):
    if isbinstr(k_input) and len(k_input) == 10:
        return k_input
    
    elif isbinstr(k_input) and len(k_input) > 10:
        return k_input[:10]
    #relleno a la derecha 
    elif isbinstr(k_input):
        return k_input + '0' * (10 - len(k_input))
...
def apply_sboxes(bits):
    ...
    row_s0 = int(left[0] + left[3], 2)
    col_s0 = int(left[1] + left[2], 2)
    s0_val = S0[row_s0][col_s0]
    s0_out = format(s0_val, '02b')
    
    # Para S1 enmedio
    row_s1 = int(right[0] + right[3], 2)
    col_s1 = int(right[1] + right[2], 2)
...
def function_f(bits, subkey):
    ...
    xor_result = ''
    for i in range(8):
        xor_result += '1' if expanded[i] != subkey[i] else '0'
...
def ENCRYPT_SDES(binary_str, k1, k2):
    ...
    nleft = ''
    for i in range(4):
        nleft += '1' if left[i] != f_result[i] else '0'
    nright = right
    
    # Intercambiamos las mitades
    ...
    # Combinamos y aplicamos la permutación inversa IP^-1
...
def DECRYPT(binary_str, k1, k2):
    ...
    # Dividimos en dos partes de 4 bits
    left, right = bits[:4], bits[4:]
    
    # Ronda 1: Aplicamos F con k2 (invertimos el orden de las subllaves)
    f_result = function_f(right, k2)
    nleft = ''
    for i in range(4):
        nleft += '1' if left[i] != f_result[i] else '0'
    ...
    # Intercambiamos las mitades
    left, right = nrifht, nleft
    
    # Ronda 2: Aplicamos F con k1
    f_result = function_f(right, k1)
    nleft = ''
    for i in range(4):
        nleft += '1' if left[i] != f_result[i] else '0'
    ...
    # Combinamos y aplicamos la permutación inversa IP^-1
    final_bits = permute(nleft + nrifht, INV_IP)
    return final_bits

# CONVERSIONES_________________________________________________________________
def text_to_bin(text):
    binary = ''
    for char in text:
        # Convertimos cada carácter a su valor ASCII y luego a binario de 8 bits
...
def encrypt_text(input_str, k1, k2):
    ...
    # dividimos en bloques de 8 bits y ciframos
    ciphertext = ''
    for i in range(0, len(binary), 8):
        block = binary[i:i+8]
...
#BASE 64_________________________________________________________________________________
# Función para codificar un string binario a base64
...
# Función para decodificar de base64 a binario
def base64_to_binary(base64_str):
    try:
        ...
        # Calculamos la longitud en bits (número de bytes * 8)
