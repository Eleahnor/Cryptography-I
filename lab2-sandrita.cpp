// Librer" << char(161) << "as utilizadas en el programa
#include <iostream>
#include <map>
#include <vector>
#include <string>
#include <fstream>  // librer" << char(161) << "a para manipular archivos
using namespace std;

/* Instrucciones de compilación
g++ lab2-sandrita.cpp -o lab2 
*/
...
void leer_alfabeto(int &n, map<char, int> &alfabeto, map<int, char> &valores) {
    ...
    cout << "[1] Ingresar como cadena de texto. \n[2] Ingresa s" << char(161) << "mbolo por s" << char(161) << "mbolo.\n";
    ...
    if(opc == '1') {
        cout << "Ingresa el tamano del alfabeto: ";
        cin >> n;
    ...
    // Leer desde un archivo
        ...
        if(!archivo) {
            cout << "\n" << char(173) << "No se encontr" << char(162) << " el archivo! :(" << endl;
            return;
        }
        ...
        cout << "\n" << char(173) << "Se carg" << char(162) << " correctamente! :)" << endl;
    }
...
// Verificamos si la longitud de la llave de cifrado y mensaje son  " << char(160) << "
...
string vigenenere_decipher(int n, map<char, int> alfabeto, map<int, char> valores) {
    ...
    while(idx < C.length()) {    // Iteramos sobre el mensaje
        for(int i=0; i<K.length() && idx < C.length(); i++) {   // Iteramos con la llave
            // Restamos valores del mensaje y la llave,  " << char(160) << " obtenemos un módulo positivo 
            // (C++ no convierte módulos a positivos  " << char(160) << ")
...
int main() {
    ...
    while(f) {
        ...
        cout << "[2] Vigen" << char(138) << "nere encipher." << endl;
        cout << "[3] Vigen" << char(138) << "nere decipher." << endl;
        ...
        switch(opc) {
            ...
            case 2:
                ...
                if(!s.empty()) 
                    ...
                    cout << char(173) << "Vuelve a intentarlo! :/" << endl;
                break;
            ...
            case 4:
                if(!alfabeto.empty()) {
                    ...
                    cout << char(173) << "El alfabeto est" << char(160) << " vac" << char(161) << "o! :/" << endl;
                }
                break;
            ...
            default:
                cout << char(173) << "Ingresa una opci" << char(162) << "n  " << char(160) << "! :(" << endl;
        }
        cout << endl;
    }
    return 0;
